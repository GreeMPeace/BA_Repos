...
namespace Net3D.Utils
{
    public static class GoogleMapsAPI
    {
        ...
        public static int GetZoom(Coordinate center, int mapWidth, int mapHeight, double[] min, double[] max)
        {
            for (int i = 0; i <= 21; i++)
            {
                MapCoordinates coord = GetBounds(center, i, mapWidth, mapHeight);
                if (coord.SouthWest.X > min[0] || coord.SouthWest.Y > min[1] || coord.NorthEast.X < max[0] || coord.NorthEast.Y < max[1])
                    return i - 1;
            }
            return 0;
        }
