...
p.createModel = function(shp, spherize, dbf) {
  var polys = [];
  var lines = [];
      ...
      for (var k=0; k<parts.length; k++) {
        ...
        if (r.type == SHP.POLYGON) {
          //console.log('new polygon', poly.length, points.length/2);
            poly.pop();
            polys.push(createGeometry(poly, dbf, i)/*new THREE.ShapeGeometry(new THREE.Shape(poly), { amount: 0 })*/);
        } else {
  ...
  for (var i=0; i<polys.length; i++) {
    model.add(new THREE.Mesh(
      polys[i],
      new THREE.MeshBasicMaterial({ color: 0x9c8dc7, /*wireframe: true*/ })
    ));
  }
...
function createGeometry(poly, dbf, i) {
    var shape = new THREE.Shape(poly);
    var height = dbf.records[i]["heightroof"];
    var path = new THREE.LineCurve3(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, height));
    return shape.extrude({ extrudePath: path });
};
